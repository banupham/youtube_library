(() => {
  ...
  function extractVideoCards(root = document, limit = 120) {
    ...
    for (const card of cards) {
      ...
      const title = validTitle(anchor.getAttribute('title'))
        || validTitle(anchor.textContent)
      ...
      output.push({
        ...
        metadata_text: text(card, [
          '#metadata-line', '#metadata', '.yt-content-metadata-view-model-wiz__metadata-text',
          '.yt-lockup-metadata-view-model-wiz__metadata'
        ]),
        duration_text: text(card, [
          'ytd-thumbnail-overlay-time-status-renderer span', '#time-status span', '.badge-shape-wiz__text'
        ])
      });
  ...
  async function quotaAllows(kind) {
    ...
    const quota = stored.passiveDailyQuota?.date === day
      ? stored.passiveDailyQuota
      : { date: day, counts: {} };
  ...
  async function sendSnapshot(kind, payload) {
    ...
    try {
      const result = await chrome.runtime.sendMessage({
        type: 'youtube_library_passive_snapshot',
        payload
      });
  ...
  async function captureHome(generation) {
    ...
    await sendSnapshot('home', {
      ...
      extraction_mode: 'passive_dom_natural_home',
      extraction_version: 'passive_v1',
  ...
  async function captureWatch(generation) {
    ...
    await sendSnapshot('up_next', {
      ...
      captured_at: new Date().toISOString(),
      page_url: location.href,
      extraction_mode: 'passive_dom_natural_watch_page',
      extraction_version: 'passive_v1',
      parent_video_id: parentVideoId,
      parent_title: String(document.title || '').replace(/\s*-\s*YouTube\s*$/, '').trim(),
  ...
  async function captureSubscriptions(generation) {
    ...
    await sendSnapshot('subscriptions', {
      ...
      captured_at: new Date().toISOString(),
      page_url: location.href,
      extraction_mode: 'passive_dom_natural_subscriptions',
      extraction_version: 'passive_v1',
      item_count: items.length,
  ...
  async function captureChannels(generation) {
    ...
    await sendSnapshot('channels', {
      ...
      captured_at: new Date().toISOString(),
      page_url: location.href,
      extraction_mode: 'passive_dom_natural_subscribed_channels',
      extraction_version: 'passive_v1',
      item_count: 0,
